{% if var('SHOPIFYV2') and var('ShopifyCustomCollections',False) %}
{{ config( enabled = True ) }}
{% else %}
...
table_pattern=var('shopify_custom_collections_tbl_ptrn','%shopify%custom_collections'),
...
{{timezone_conversion("a.published_at")}} as published_at,
a.* except(id,updated_at,published_at),
