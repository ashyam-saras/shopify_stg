{% if var('SHOPIFYV2') and var('ShopifyEvents',False) %}
{{ config( enabled = True ) }}
{% else %}
...
table_pattern=var('shopify_events_tbl_ptrn','%shopify%events%'),
exclude=var('shopify_events_exclude_tbl_ptrn','%fulfillment%'),
