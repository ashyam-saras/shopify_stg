{% if var('SHOPIFYV2') and var('ShopifyPriceRules',False) %}
    {{ config(enabled=True) }}
{% else %}
...
table_pattern=var('shopify_price_rules_tbl_ptrn','%shopify%price_rules'),
...
from (
    ...
    a.* except(id,starts_at,ends_at,created_at,updated_at),
