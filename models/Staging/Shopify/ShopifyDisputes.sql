{% if var('SHOPIFYV2') and var('ShopifyDisputes',False) %}
{{ config( enabled = True ) }}
{% else %}
...
table_pattern=var('shopify_disputes_tbl_ptrn','%shopify%disputes'),
...
from (
    ...
    a.* except(id,evidence_due_by,evidence_sent_on,finalized_on,initiated_at),
    dense_rank() over (partition by a.id order by a.{{daton_batch_runtime()}} desc) as row_num
    from union_tables a
    ) b
{% if var('currency_conversion_flag') %}
    left join {{ref('ExchangeRates')}} c on date(b.finalized_on) = c.date and b.currency = c.to_currency_code
{% endif %}
where row_num=1