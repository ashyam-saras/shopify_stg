{% if var('SHOPIFYV2') and var('ShopifyPolicies',False) %}
    {{ config(enabled=True) }}
{% else %}
...
table_pattern=var('shopify_policies_tbl_ptrn','%shopify%policies'),
...
from (
    ...
    a.* except(title,created_at,updated_at),
    dense_rank() over (partition by coalesce(a.title, 'N/A') order by a.{{daton_batch_runtime()}} desc) as row_num
    from union_tables a
    ) b
where row_num=1
