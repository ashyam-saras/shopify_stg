{% if var('SHOPIFYV2') and var('ShopifyAbandonedCheckouts',False) %}
{{ config( enabled = True ) }}
{% else %}
...
table_pattern=var('shopify_abandoned_checkouts_tbl_ptrn','%shopify%abandoned_checkouts%'),
exclude=var('shopify_abandoned_checkouts_exclude_tbl_ptrn','%transactions%'),
...
concat(split(lower(replace(split(_dbt_source_relation,'.')[2],'`','')),'_shopify')[0], 
'_',
...
{{timezone_conversion("a.completed_at")}} as completed_at,
a.* except(id,created_at , updated_at, completed_at),
row_number() over (partition by a.id order by a.{{daton_batch_runtime()}} desc) as row_num
from 
union_tables a 
) b 
{% if var('currency_conversion_flag') %}
    left join {{ref('ExchangeRates')}} c on date(b.updated_at) = c.date and b.currency = c.to_currency_code
{% endif %}
where row_num=1
