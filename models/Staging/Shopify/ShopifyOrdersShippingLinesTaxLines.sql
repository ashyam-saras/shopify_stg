...
ifnull({{extract_nested_value("tax_lines","price","numeric")}},0) as shipping_lines_tax_lines_price,
ifnull({{extract_nested_value("tax_lines","rate","numeric")}},0) as shipping_lines_tax_lines_rate,
ifnull({{extract_nested_value("tax_lines","title","string")}},'') as shipping_lines_tax_lines_title,
{{extract_nested_value("tax_lines","channel_liable","string")}} as shipping_lines_tax_lines_channel_liable,
...
where {{extract_nested_value("tax_lines","price","numeric")}} is not null
qualify row_number() over (partition by a.id,shipping_lines_id,shipping_lines_tax_lines_price,shipping_lines_tax_lines_rate,shipping_lines_tax_lines_title,shipping_lines_tax_lines_channel_liable order by a._daton_batch_runtime desc) = 1